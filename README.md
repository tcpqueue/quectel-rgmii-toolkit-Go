...
后台默认地址192.168.225.1，账号密码都是admin，控制台账号root 密码admin（首次安装默认，之后使用系统 root 密码）
...
| `authenticateNativeConsole()` | 控制台登录认证，账号为 `root`，密码为 `admin（首次安装默认，之后使用系统 root 密码）`，最多允许 3 次失败尝试。 |
...
浏览器 /#console
  ...
  -> 控制台账号 root / admin（首次安装默认，之后使用系统 root 密码） 认证
...
安装脚本通过后端 `root-password-init` 初始化系统 root 密码为 `admin`，完成后保存 `root-password.initialized` 标记；升级和重新安装遇到该标记时不重置已修改的系统密码。Web 登录账号仍独立为 `admin`。在“系统设置”中，Web 登录密码和系统 root 密码分别修改，系统改密需要当前 root 密码；成功后撤销旧 Web 会话及控制台连接。控制台认证使用系统 `/etc/shadow`，不再使用固定口令。Linux 通过 OpenSSL 验证密码哈希、通过系统 `passwd -a sha512 root` 更新密码，明文密码仅通过标准输入传递。

安装时应用目录和 `simpleadmin-httpd` 程序权限为 777，凭据文件仍保持受限权限，不递归将密码文件设为 777。ADB 安装包解压到 `/tmp/development` 后执行安装脚本；WebUI 也可通过 `adb forward tcp:18081 tcp:80` 在电脑本地访问 `http://127.0.0.1:18081/`。
